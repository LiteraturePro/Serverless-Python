...
class Task(object):
    ...
    def diyText(self):
        ...
        kaoyan_day = datetime.date(2020,7,16) # 倒计时
    ...
    def start(self):
        try:
            ...
            self.sign()
            self.detail()
            for i in range(1,5): # 5表示刷4次310首歌，根据需求自行修改后面的5
                self.daka()
                self.log('用户:' + self.name + '  第' + str(i) + '次打卡成功,即将休眠15秒')
                logger.info('用户:' + self.name + '  第' + str(i) + '次打卡成功,即将休眠15秒')
                time.sleep(15) #休眠时间（每次打卡间隔时间），根据需求自行修改
            self.server()
        except:
