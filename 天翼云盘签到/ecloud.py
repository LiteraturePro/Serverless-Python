# !/usr/bin/env python
# coding=utf-8
import sys, requests, time, re, rsa, json, base64, time, pytz, datetime
from urllib import parse
from io import StringIO

s = requests.Session()

# Python版本 3.6，Forked from "https://github.com/mengshouer/Cloud189Checkin"
# 请依次修改 13、14、15行中的需要修改的部分内容
# 天翼云盘每日签到一次，抽奖2次，添加SERVER酱签到的微信推送
...
def pushWechat(desp,nowtime):
    if '失败' in desp :
        params = {
            'text': '天翼云盘签到失败提醒' + nowtime,
            'desp': desp
            }
    else:
        params = {
            'text': '天翼云盘签到提醒' + nowtime,
            'desp': desp
    }
